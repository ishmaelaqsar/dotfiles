...
# The package table, so the summary at the end names the rows install.sh
# installs, from one source.
...
# Same guard as install.sh: if a different checkout owns this home, this
# repository installed nothing here. To remove fonts or config would break that
# setup.
...
# systemd user units — disable ours before the symlink pass removes the unit
# files, and undo the SSH agent handover and the podman socket install.sh set up
...
if command -v git >/dev/null 2>&1; then
    ...
    # Both lists, so every key install.sh writes comes back out.
...
if [ "$ALL" -eq 1 ]; then
    ...
    # symlink pass removed it. The emacs-plus tap stays.
    __run rm -rf "$HOME/.config/emacs/elpa" "${XDG_STATE_HOME:-$HOME/.local/state}/emacs"
    __run rm -f "$HOME/.local/bin/uv" "$HOME/.local/bin/uvx"
    # uv tool shims are symlinks into ~/.local/share/uv, which is gone, so they
    # dangle. Match on the target: to delete every dangling link in ~/.local/bin
    # would take the ones other tools own as well.
...
echo "  setup-emacs.sh: emacs (emacs-plus@31 on macOS, and its tap)"
